...
# Add your Slack API tokens here
BOT_TOKEN = ""
GROUP_TOKEN = ""

# Replace 'bob' with the slack username.
MESSAGE_QUERY = "from:bob"
...
def _load_db():
    ...
    Return s a dictionary keyed by unique message permalinks.
    """
    try:
...
def _store_db(obj):
    """
    Takes a dictionary keyed by unique message permalinks and writes it to the JSON 'database'
    on disk.
    """
    with open('message_db.json', 'w') as json_file:
...
def update_corpus(sc, channel):
    ...
    sc.rtm_send_message(channel, "Starting up...")
